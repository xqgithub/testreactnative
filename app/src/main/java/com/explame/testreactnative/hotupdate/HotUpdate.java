package com.explame.testreactnative.hotupdate;

import android.content.Context;

import com.explame.testreactnative.constants.AppConstant;
...
//                boolean result = (Boolean) ACache.get(context).getAsObject(AppConstant.FIRST_UPDATE);
    ...
    private static void mergePatAndAsset(Context context) {
        ...
        try {
            // 将补丁文件写入到某个位置
            com.explame.testreactnative.utils.FileUtils.writeFileFromString(FileConstant.JS_PATCH_LOCAL_FILE, patchesStr, false);
        } catch (Exception e) {
            e.printStackTrace();
        }
